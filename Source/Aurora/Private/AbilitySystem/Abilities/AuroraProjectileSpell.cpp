...
FString UAuroraProjectileSpell::GetDescription(int32 Level)
{
	
	const int32 Damage = DamageTypes[FAuroraGameplayTags::Get().Damage_Fire].GetValueAtLevel(Level);
	...
	else
	{
		return FString::Printf(
			...
			"<Default> fire damage with chance to burn</>\n\n"
		  	"<Small>Level: </>"
		  	"<Level>%d</>"),
		  	FMath::Min(Level, ProjectilesNum), Damage, Level);
	}
	
}

FString UAuroraProjectileSpell::GetNextLevelDescription(int32 Level)
{
	const int32 Damage = DamageTypes[FAuroraGameplayTags::Get().Damage_Fire].GetValueAtLevel(Level);
